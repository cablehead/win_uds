use super::*;
use std::{io, net::Shutdown, path::Path, ptr};

use windows::Win32::Networking::WinSock::{
    self, ADDRESS_FAMILY, AF_UNIX, INVALID_SOCKET, SEND_RECV_FLAGS, SOCK_STREAM, SOCKADDR_UN,
    SOCKET_ERROR, WSA_FLAG_OVERLAPPED,
};

pub struct UnixStream(WinSock::SOCKET);
impl UnixStream {
    pub fn connect<P: AsRef<Path>>(path: P) -> io::Result<Self> {
        unsafe {
            startup()?;
            let mut connect_socket =
                WinSock::socket(AF_UNIX as _, SOCK_STREAM, WSA_FLAG_OVERLAPPED as _)?;
            ...
            ptr::copy_nonoverlapping(bytes.as_ptr() as _, &mut addr.sun_path, bytes.len());
            let err = WinSock::connect(
                connect_socket,
                &addr as *const _ as *const _,
                addr.sun_path.len() as _,
            );
            if err == SOCKET_ERROR {
                WinSock::closesocket(connect_socket);
                connect_socket = INVALID_SOCKET;
            }
            Ok(Self(connect_socket))
        }
    }
